...
import { DocumentNode, useMutation, useQuery, useReactiveVar } from '@apollo/client';
...
const AgentDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	...
	const {
		loading: getMemberPropertiesLoading,
		data: getMemberPropertiesData,
		error: getMemberPropertiesError,
		refetch: getMemberPropertiesRefetch,
	...
	const {
		loading: getCommnetsLoading,
		data: getCommnetsData,
		error: getCommnetsError,
		refetch: getCommnetsRefetch,
	...
	useEffect(() => {
		if (searchFilter.search.memberId) {
			getMemberPropertiesRefetch({ variables: { input: searchFilter } }).then();
		}
	}, [searchFilter]);

	useEffect(() => {
		if (commentInquiry.search.commentRefId) {
			getCommnetsRefetch({ variables: { input: commentInquiry } });
		}
	}, [commentInquiry]);
	...
	const createCommentHandler = async () => {
		try {
			...
			await getCommnetsRefetch({ input: commentInquiry });
		} catch (err: any) {
	...
	const likePropertyHandler = async (user: T, id: string) => {
		try {
			...
			await getMemberPropertiesRefetch({ input: searchFilter });
