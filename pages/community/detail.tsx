...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
	...
	const {
		loading: boarArticlesLoading,
		data: boarArticlesData,
		error: boarArticlesError,
		refetch: boarArticlesRefetch,
	...
	const likeBoardArticleHandler = async (user: T, id: string) => {
		try {
			...
			await boarArticlesRefetch({ input: articleId });
	...
	const creteCommentHandler = async () => {
		...
		try {
			...
			setComment('');
			sweetMixinSuccessAlert('Successfuly commented');
		} catch (err: any) {
